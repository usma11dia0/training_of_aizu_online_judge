...
# preorderとinorderリストは、それぞれ以下の特性を持っています：
# preorderリストでは、根（root）要素が最初に現れ、次に左部分木の要素が、その後に右部分木の要素が続きます。
# inorderリストでは、左部分木の要素が最初に並び、次に根要素、その後に右部分木の要素が続きます。
# これらの特性を元に、inorderリストから根要素の位置（index）を見つけることで、左部分木と右部分木に含まれる要素を判別できます。つまり、inorderリストのindexより左側にある要素は左部分木、右側にある要素は右部分木を構成します。
# その後、preorderリストをinorderのindexで分割することにより、左部分木と右部分木のそれぞれに対応するpreorderリストを得ることができます。なぜなら、preorderリストの最初の要素は常に根要素であり、その後に続くのは左部分木、その後に右部分木の要素が続くからです。
# 具体的には、preorderリストでは最初の要素（根）を除くと、次のindex個の要素が左部分木、その後の要素が右部分木を表します。これらの部分リストを利用して、左部分木と右部分木のそれぞれに対して再帰的に同じ操作を行うことで、全体の二分木を構築していきます。