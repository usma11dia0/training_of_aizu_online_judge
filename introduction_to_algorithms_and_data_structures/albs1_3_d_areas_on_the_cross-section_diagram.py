...
for index, a in enumerate(A):
    if a == "\\":
        ...
        if len(stack_back_slash) > 0:
            ...
            # 導出した同一高位水量の'\'のインデックスが、
            # 既出の水たまりの'\'のインデックスよりも大きくなる(右側にくる)まで
            # 両者の結合を繰り返す
            while True:
...
result_puddle = deque()
result_puddle.append(str(len(stack_puddle)))
for puddle in stack_puddle:
    result_puddle.append(str(puddle[1]))
print(" ".join(result_puddle))
